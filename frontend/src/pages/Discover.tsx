...
export default function Discover() {
  ...
  const handleImport = useCallback(
    async (r: SearchResult) => {
      ...
      if (folders.length === 0) {
        alert(
          "Please link a local folder in your Library before downloading tracks.",
        );
        return;
      }
    ...
    return (
      <div className="flex flex-col rounded-lg overflow-hidden border border-white/[0.07] bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
        ...
        >
          ...
          {result.duration > 0 && (
            <span className="absolute bottom-1 right-1.5 text-[10px] font-mono bg-black/70 text-white px-1 py-0.5 rounded">
        ...
        <div className="flex flex-col flex-1 p-2 gap-1.5">
          ...
          <div className="mt-auto pt-0.5">
            {isDone ? (
              ...
              <div
                className="text-[10px] text-red-400 text-center py-1 truncate"
                title={importState.message}
              >
                ✗ {importState.message || "Failed"}
              </div>
            ) : isActive ? (
              <div className="text-[11px] text-purple-400 font-medium text-center py-1 animate-pulse">
              ...
              <button
                ...
                className="w-full py-1 rounded bg-white/5 hover:bg-purple-600 hover:text-white border border-white/10 hover:border-purple-500 transition-all text-[11px] text-gray-300"
              >
                Download & Import
              </button>
            )}
  ...
  function EmbeddedPlayer({ video, onClose }: any) {
    ...
    return (
      ...
      >
        <div
          className="relative w-full max-w-2xl mx-4 rounded-xl overflow-hidden border border-white/10 shadow-2xl bg-[#0d0d0d]"
        ...
        >
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/[0.07]">
            ...
            <button
              onClick={onClose}
              className="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors text-sm"
            >
              ✕
            </button>
          </div>
          ...
          <div className="flex-1 max-w-xl">
            <form onSubmit={handleSearch} className="relative group">
              ...
              <input
                ...
                className="w-full bg-white/[0.03] border border-white/[0.07] rounded-lg pl-9 pr-24 py-1.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.05] transition-all"
              />
              <button
                type="submit"
                disabled={isSearching || !query.trim()}
                className="absolute inset-y-1.5 right-1.5 px-3 rounded text-[10px] font-medium text-white transition-colors flex items-center bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:hover:bg-purple-600"
              >
                Search
              </button>
            </form>
          ...
          {results.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 pb-12 gap-3">
              {isSearching ? (
                ...
                <>
                  <div className="w-12 h-12 rounded-full border border-white/5 flex items-center justify-center text-xl">
                    ♪
                  </div>
